# Code provided by Simon Steinlin, Sayan Chaki, and Thierry Fournel
# Laboratoire H. Curien - University of Saint-Etienne (France)
...
# Python implementation of the ST-MLP model for change detection proposed in 
#the paper entitled "In-place change detection and restitution in one-to-one-class image comparison" 
# Part 1/2: Training ST-MLP architecture 
...
#
#main_dir = r'C:\Users\fournel\Desktop\_WORK\_PROJECTS\ON\ROIi\_COMMUNICATIONS\JImaging2022\codes\Data\retrieval1474'
#main_dir = r'U:\MesDocuments\lettrines_ajust'
main_dir = r'U:\MesDocuments\FleuronsBlocs'
path_train = os.path.join(main_dir,'training')
L_train = len(os.listdir(path_train))
img_train = np.zeros((L_train,128,128))

for i,filename in enumerate(os.listdir(path_train)) :
    img_train[i,:,:] = np.mean(cv2.imread(os.path.join(path_train,filename)),axis=-1)/255
img_train = img_train.reshape((-1,128,128,1))

inputs = Input(shape=img_train[0].shape)
...
model.fit(x = img_train, y = img_train,epochs = 10, batch_size = 32)
...
model2.load_weights('./weights/model')
#
   
output = model2.predict(img_test)

filename = r'C:\Users\fournel\Desktop\_WORK\_PROJECTS\ON\ROIi\_COMMUNICATIONS\JImaging2022\codes\Data\out1.jpg'
cv2.imwrite(filename, 255*output[0])
# filename = r'C:\Users\fournel\Desktop\_WORK\_PROJECTS\ON\ROIi\_COMMUNICATIONS\JImaging2022\codes\Data\out2.jpg'
# cv2.imwrite(filename, 255*output[1])
# filename = r'C:\Users\fournel\Desktop\_WORK\_PROJECTS\ON\ROIi\_COMMUNICATIONS\JImaging2022\codes\Data\out3.jpg'
